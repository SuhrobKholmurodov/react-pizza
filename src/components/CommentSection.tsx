...
export const CommentSection = ({ reviews, filteredReviews }: CommentSectionProps) => {
  const { t, lng } = useLocalization();
  const currentFilter = useSelector(selectCommentFilter);

  const getReviewCount = () => {
    if (currentFilter === CommentFilterEnum.RECOMMEND) {
      return reviews.filter((review) => review.recommendation).length;
    } else if (currentFilter === CommentFilterEnum.NOT_RECOMMEND) {
      return reviews.filter((review) => !review.recommendation).length;
    } else {
    ...
    >
      <div className="font-[700] sm:dark:mt-[-10px] sm:flex-col flex sm:items-start items-center dark:bg-mainBgColor justify-between bg-white rounded-[12px] gap-[5px] mt-[15px] p-[10px]">
        <div className="flex items-center dark:text-mainTextColor gap-[3px] text-[20px]">
          ...
          <p className="font-[550]">({getReviewCount()})</p>
        </div>
          ...
          filteredReviews.map((review, index) => (
            <li key={index} className="flex gap-[15px] mb-[20px]">
              ...
              <div className="flex flex-col">
                <p className="text-[16px] w-[470px] sm:w-full flex items-center justify-between font-[600] text-[gray]">
                  ...
                  <Rating
                    ...
                    sx={{
                      fontSize: '19px',
                      '& .MuiRating-iconEmpty': {
                        color: document.body.classList.contains('dark-mode') ? '#fff' : 'grey',
                      },
                    }}
                  />
                </p>
                ...
                <p className="text-[12px] text-[gray] mt-[5px]">{review.date[lng]}</p>
              </div>
            </li>
          ))
        ) : (
